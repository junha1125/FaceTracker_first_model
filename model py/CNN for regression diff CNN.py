...
for i in range(plt_row*plt_col):
    ...
    j=random.randrange(0,len(x_train_diff)-1)
    sub_plt = axarr[i//plt_row, i%plt_col]
    sub_plt.axis('off')
    sub_plt.imshow(x_train_diff[j].reshape(128,128))
...
with K.tf.device('/gpu:0'):
    ...
    hist_256 = model.fit(x_train_diff, y_train, validation_split=0.15 ,
                         batch_size=512, epochs=epochs,verbose=2 )



model.save('C:/Users/admin/Desktop/model_diff_yolo_CNN.h5')
...
# 7. 모델 사용
yhat_train_val = model.predict(x_train_diff, batch_size= 128)
...
for i in range(plt_row * plt_col):
    ...
    j=random.randrange(0,len(x_train_diff)-1) # 이미지 랜덤추출
    
    sub_plt = axarr[i // plt_row, i % plt_col]
    sub_plt.axis('off')
    sub_plt.imshow(x_train_diff[j].reshape(128,128))
    sub_plt.set_title('Real yaw %.1f \n Predict yaw %.1f '
                      % (y_train[j], yhat_train_val[j]))
